...
module.exports.run = async (bot, message, args) => {
  if (!message.member.hasPermission("ADMINISTRATOR"))
    return utils.timed_msg(
      utils.no_perm(
        `${message.author}, you do have the permissions to run this command!`
      ),
      5000
    );
  ...
  if (!command)
    return utils.timed_msg(
      utils.cmd_fail(
        "Please enter a command to reload!",
  ...
  return [
    message.delete(),
    utils.timed_msg(`<:ThinkEyes:483713711572516874> ${command}.js has been reloaded successfully!`, 5000)
  ];
};
